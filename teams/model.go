...
func (t Team) GetUsersList(users *[]TblUser, offset, limit int, filter Filters, flag bool, DB *gorm.DB) ([]TblUser, int64) {
	...
	if strings.ToLower(filter.Keyword) == "all users" {

		filter.Keyword = "0"

	} else if strings.ToLower(filter.Keyword) == "users only" {
	...
	if filter.Keyword != "" {
		...
		if filter.Keyword == "0" || filter.Keyword == "1" {
			...
			query = query.Debug().Where(" tbl_users.data_access=?", filter.Keyword)
...
func (t Team) DeleteMultipleUser(user *TblUser, usersId []int, DB *gorm.DB) error {
	...
	if err := DB.Model(&TblUser{}).Where("id IN (?)", usersId).Updates(map[string]interface{}{"is_deleted": user.IsDeleted, "deleted_on": user.DeletedOn, "deleted_by": user.DeletedBy}).Error; err != nil {
		// TblUser{IsDeleted: user.IsDeleted, DeletedOn: user.DeletedOn, DeletedBy: user.DeletedBy}
...
func (t Team) GetAdminRoleUsers(roleid []int, DB *gorm.DB) (userlist []TblUser, err error) {
	...
	if err := DB.Model(TblUser{}).Where("role_id in (?) and is_active=1 and is_deleted=0", roleid).Find(&userlist).Error; err != nil {
